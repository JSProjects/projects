...
<SCRIPT language="javascript"><!--
        function addRow(tableID) {
            ...
            var rowCount = Number(table.rows.length -1);
            strUser=Number(strUser);
            var number=rowCount+strUser;
           for(rowCount; rowCount<number; rowCount++) {
            		...
            		cell1.innerHTML = rowCount+1;
           			 ...
           			 element2.size="30";
           			 var length=rowCount;
            		 element2.name = "serialNumbers["+length+"].name";
            		...
            		element3.size="40";
            		var length=rowCount;
            		element3.name = "hmids["+length+"].name";
        ...
        function check(tableID)
        {
           ...
           //	var goodColor = "#008000";
           	if(endUser == null || endUser == ""){
           		document.getElementById("endUser").style.backgroundColor = badColor;
           	    message.innerHTML = "Please input a valid End User Name";
           	} 
           	
        	var table = document.getElementById(tableID);
        	var rowCount = Number(table.rows.length);
        	var message = document.getElementById('confirmMessage');
        	var nfrkeys = document.forms["frm"]["keys"].value;
        	nfrkeys = nfrkeys.split(",");
        	for(var i=1; i<=rowCount-1; i++) {
        		var row=table.rows[i];
        		var sno=row.cells[1].childNodes[0].value;
               	var pattern = /^\d{14}$/;
               	//SerialNumber Validation
                   if(sno!=null && sno!=""){
                   	var numbers = /^[0-9]+$/;
                   	if(!sno.match(numbers)){
                   	    message.innerHTML = "Input Numeric Value :" +sno;
                   	}else if(!pattern.test(sno)) {
                   		message.innerHTML ="Expecting 14 digits :" +sno ;
                   	}else if(nfrkeys.indexOf(sno)==-1){
                   		message.innerHTML ="Not a valid NFR SerialNumber :" +sno ;
                   	}
                   	else{
                   		message.innerHTML = "";
                   	}
...
<form:form name="frm" commandName="partner" action="/NFR/provision" method='POST'>
<section>

<!-- 
<div id="test"> -->
<h2>Request NFR Keys</h2>
 <p>Submit your NFR Keys requests by providing valid Serial Numbers by VHMID (HMOL) or SystemID (HM On-prem).  
 Allow up to 24 hours for the system to validate all the Serial Numbers are NFR kits. 
 The system will then email the EK&#8217;s grouped by the unique VHMID&#8217;s and SystemID&#8217;s.</p>
...
<table class="sell" align="center">
<tr>
<td class="left">
<input type="hidden" name="keys" value="${nfrkeys}"/>
</td>
</tr>
<tr>
<td class="left">
<label>End User:</label>
<input type="text" size="40" path="endUser" name="endUser" placeholder="Enter End User Name"/>
</td>
</tr>
...
<tr>
	...
	<TABLE id="dataTable" width="350px" border="1" class=imagetable required>
	  <tr>
            <th>S.No</th>
            <th>SerialNumber</th>
            <th>VHM ID/System ID</th>
        </tr>
	   <TR>	
	  	   <TD> 1 </TD>		         
	       <TD><INPUT type="text" size="30" name="serialNumbers[0].name"/></TD>
	       <TD> <INPUT type='text' size="40" name = "hmids[0].name"/> </TD>					           
	   </TR>
	</TABLE>
	</td>
</tr>
<tr>
<td class="right">
<INPUT type="button" value="Add Rows" onclick="addRow('dataTable')" />	
	<select id="ddlViewBy" name="count">
...
<tr><td class="left">
<label>Hive Manager Type:</label>
<input type="radio" name="hmType" path="hmType" value="hmol">HMOL
<input type="radio" name="hmType" path="hmType" value="hmop" checked>HM-On Prem </td>
</tr>
<tr>
<td>
<INPUT type="button" value="Submit" class="button_example" onclick="check('dataTable')" />  </td>
</tr>
<tr>
<td><span id="confirmMessage" style="color:red;font-weight: normal;"></span></td>
</tr>
</table>
